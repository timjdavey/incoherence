...
class TestErgodic(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        cases = [
            ([np.random.power(5,samples)*10 for c in range(ensembles)], 
                [3.844483389200944, 3.889014934030624, 0.044531544829680136, 0.000679033990491004]),
            ([np.random.uniform(0,10,samples) for c in range(ensembles)], 
                [4.554780388320652, 4.605119956305231, 0.050339567984578615, 0.0005605801767564403]),
        ]
    ...
    def test_dicts(self):
        ...
        measures = [3.125397481164248, 3.198293599997641, 0.07289611883339298, 0.003254010418713944]
